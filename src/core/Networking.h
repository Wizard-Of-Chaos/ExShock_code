...
const float ORIENTATION_RES = 1/16384.0f;
...
const float MAX_SHIELDS = 100.0f;
const float SHIELDS_RES = 1 / 32.0f;

const float MIN_HEALTH_REGEN = -100.0f;
const float MAX_HEALTH_REGEN = 100.0f;
const float HEALTH_REGEN_RES = 1 / 32.0f;

const float MIN_RECHARGE_RATE = -100.0f;
const float MAX_RECHARGE_RATE = 100.0f;
const float RECHARGE_RATE_RES = 1 / 32.0f;

// This one should DEFINITELY never change
const float MIN_RECHARGE_DELAY = 0.0f;
const float MAX_RECHARGE_DELAY = 10.0f;
...
const uint32_t MAX_INT_VALUE_ORIENTATION = static_cast<uint32_t>(ceil(sqrt(2) / ORIENTATION_RES));
...
const size_type RBC_BITS_NEEDED = 3 * POSITION_BITS_NEEDED + 3 * VELOCITY_BITS_NEEDED + 3 * ORIENTATION_BITS_NEEDED + 3 * ANGULAR_VELOCITY_BITS_NEEDED + 2; // + 2 for the quat index
...
const size_type MAP_GEN_OBSTACLE_BITS_NEEDED = 2 + 13 * 8 * sizeof(uint32_t) + 8 * sizeof(uint32_t) * sizeof(MapGenObstacle::extraFloats) / sizeof(f32) + 8 * sizeof(flecs::entity_t);
...
const size_type MAP_GEN_SHIP_BITS_NEEDED = 7 
	...
	+ 3 * 8 * sizeof(flecs::entity_t) 
	+ 8 * sizeof(flecs::entity_t) * sizeof(MapGenShip::hardpointEntities) / sizeof(flecs::entity_t);
const size_type MAP_GEN_SHIP_BYTES_NEEDED = ceil(MAP_GEN_SHIP_BITS_NEEDED / 8.0);

const size_type NETWORK_SHOT_BITS_NEEDED = 4 * 8 * sizeof(flecs::entity_t) 
	+ 3 * 8 * sizeof(uint32_t) * sizeof(Network_ShotFired::directions) / sizeof(vector3df) 
	+ 8 * sizeof(flecs::entity_t) * sizeof(Network_ShotFired::projIds) / sizeof(flecs::entity_t);
const size_type NETWORK_SHOT_BYTES_NEEDED = ceil(NETWORK_SHOT_BITS_NEEDED / 8.0);

const u32 HOST_RANGE = 65536;
const u32 CONNECTED_RANGE = 16384;
...
{
	ecs_entity_t entity = INVALID_ENTITY_ID;
	uint32_t accumulatedPriority = 0;
	AccumulatorEntry() {}
	AccumulatorEntry(flecs::entity ent) : entity(ent.id()) {}
};
...
void initializeNetworkingComponent(flecs::entity ent, uint16_t priority = 1);
