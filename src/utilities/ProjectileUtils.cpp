...
ProjectileInfoComponent _getProjectileInfo(const WeaponInfoComponent* wepInfo, const WeaponFiringComponent* wepFire)
{
	...
	projectileInfo.multiplier = wepInfo->multiplier;
	...
	std::function<void(flecs::entity, const WeaponInfoComponent*, const WeaponFiringComponent*, vector3df)> fx_func = _addLightShaftFx, Network_ShotFired* chaser=nullptr)
{
	...
	vector3df dir;
	vector3df spawnPos;
	NetworkId ids[MAX_PROJECTILES_PER_SHOT];
	vector3df directions[MAX_PROJECTILES_PER_SHOT];

	for (u32 i = 0; i < wep->projectilesPerShot; ++i) {
		NetworkId shotId = INVALID_NETWORK_ID;
		if (chaser) {
			shotId = chaser->projIds[i];
		}
		auto proj = _getProjectileEntity(wep, fire, wepId, owner, isTurret);
		...
		if (gameController->isNetworked()) {
			initializeNetworkingComponent(proj, 5, shotId);
			auto net = proj.get_mut<NetworkingComponent>();
			ids[i] = proj.get<NetworkingComponent>()->networkedId;
		}
		ids[i] = proj;
		directions[i] = dir;
	}
	if (!chaser && gameController->isNetworked()) {//this was a shot generated by the client; we need to send it
		...
		shot.firingWeapon = wepId.get<NetworkingComponent>()->networkedId;
		shot.spawn = spawnPos;
		for (u32 i = 0; i < wep->projectilesPerShot; ++i) {
...
void _buffFx(WeaponInfoComponent* wep, ISceneNode* ship, u32 duration = 8000U, std::string which = "assets/effects/buff_defense")
{
	if (!ship) return; //??
	IBillboardSceneNode* node = smgr->addBillboardSceneNode(ship, dimension2df(wep->radius, wep->radius), vector3df(0), ID_IsNotSelectable);
	node->setColor(wep->projectileLightColor);
	node->setMaterialType(EMT_TRANSPARENT_ALPHA_CHANNEL);
	node->setMaterialFlag(EMF_LIGHTING, false);
	ISceneNodeAnimator* anim = getTextureAnim(which, 12, true, false);
	node->addAnimator(anim);
	anim->drop();
	anim = smgr->createDeleteAnimator(duration);
	node->addAnimator(anim);
	anim->drop();
}
...
bool wepFire_goron(WeaponInfoComponent* wep, WeaponFiringComponent* fire, PowerComponent* power, flecs::entity wepId, f32 dt, Network_ShotFired* chaser)
{
	...
	if (ship.has<IrrlichtComponent>()) {
		...
		_buffFx(wep, irr->node, 8000U, "assets/effects/buff_defense");
	}
	fire->hasFired = true;
	return true;
}

bool wepFire_electron(WeaponInfoComponent* wep, WeaponFiringComponent* fire, PowerComponent* power, flecs::entity wepId, f32 dt, Network_ShotFired* chaser)
{
	if (!_parentCheck(wepId)) return false;
	flecs::entity ship = wepId.target(flecs::ChildOf);
	...
	if (ship.has<IrrlichtComponent>()) {
		auto irr = ship.get<IrrlichtComponent>();
		_buffFx(wep, irr->node, 8000U, "assets/effects/buff_offense");
	}
	fire->hasFired = true;
	return true;
}

bool wepFire_alienHealth(WeaponInfoComponent* wep, WeaponFiringComponent* fire, PowerComponent* power, flecs::entity wepId, f32 dt, Network_ShotFired* chaser)
{
	if (!_parentCheck(wepId)) return false;
	flecs::entity ship = wepId.target(flecs::ChildOf);
	if (ship.has<HealthComponent>()) {
	...
	if (ship.has<IrrlichtComponent>()) {
		auto irr = ship.get<IrrlichtComponent>();
		_buffFx(wep, irr->node, 10000U, "assets/effects/radiation");
	}
	fire->hasFired = true;
	return true;
}

void _shieldImplodeCallback(flecs::entity ent)
{
	if (!ent.has<IrrlichtComponent>()) return;
	audioDriver->playGameSound(ent, "death_explosion_mine.ogg");
	explode(ent.get<IrrlichtComponent>()->node->getPosition(), 1.f, 25.f, 400.f, 0.f, 500.f, EXTYPE_TECH);
}

bool wepFire_physShield(WeaponInfoComponent* wep, WeaponFiringComponent* fire, PowerComponent* power, flecs::entity wepId, f32 dt, Network_ShotFired* chaser)
{
	if (!_parentCheck(wepId)) return false;
	flecs::entity ship = wepId.target(flecs::ChildOf);
	if (ship.has<BulletRigidBodyComponent>() && ship.has<IrrlichtComponent>() && ship.has<ThrustComponent>()) {
		auto rbc = ship.get<BulletRigidBodyComponent>();
		auto irr = ship.get<IrrlichtComponent>();
		btVector3 scale = irrVecToBt(irr->node->getScale());
		btVector3 shieldPos = rbc->rigidBody->getCenterOfMassPosition() + (getRigidBodyForward(rbc->rigidBody) * 65.f * scale);
		vector3df irrPos = btVecToIrr(shieldPos);

		btQuaternion orient = shortestArcQuat(irrVecToBt(vector3df(0, -1, 0)), getRigidBodyForward(rbc->rigidBody));

		btVector3 zero = btVector3(0.f, 0.f, 0.f);
		flecs::entity proj = game_world->entity().child_of(ship);

		vector3df rotvec;
		orient.getEulerZYX(rotvec.Z, rotvec.Y, rotvec.X);

		auto pos = btVecToIrr(shieldPos);

		BulletRigidBodyComponent shotRBC;

		btTransform transform;
		transform.setIdentity();

		transform.setRotation(orient);
		transform.setOrigin(shieldPos);
		auto motionState = new btDefaultMotionState(transform);
		shotRBC.shape = new btBoxShape(btVector3(wep->radius*2.f, .25f, wep->radius*2.f));
		btVector3 localInertia;

		shotRBC.shape->calculateLocalInertia(rbc->rigidBody->getMass(), localInertia);
		shotRBC.rigidBody = new btRigidBody(7.5f, motionState, shotRBC.shape, localInertia);
		setIdOnBtObject(shotRBC.rigidBody, proj);
		bWorld->addRigidBody(shotRBC.rigidBody);

		btTransform A = rbc->rigidBody->getCenterOfMassTransform().inverse();
		btTransform B = shotRBC.rigidBody->getCenterOfMassTransform().inverse();

		IrrlichtComponent projIrr;
		auto node = smgr->addVolumeLightSceneNode(0, ID_IsNotSelectable, 1024, 1024, SColor(150, 20, 20, 20), SColor(0,0,0,0),
			btVecToIrr(shieldPos), rotvec);
		node->setScale(vector3df(wep->scale, wep->length, wep->scale));
		node->setMaterialFlag(EMF_LIGHTING, false);
		node->setMaterialType(EMT_TRANSPARENT_ADD_COLOR);

		projIrr.node = node;
		projIrr.node->setName(idToStr(proj).c_str());

		auto cleanup = smgr->createDeleteAnimator((u32)(wep->lifetime * 1000.f) + 25);
		projIrr.node->addAnimator(cleanup);
		cleanup->drop();

		auto anim = getTextureAnim("assets/effects/buff_shield/", 16, true);
		projIrr.node->addAnimator(anim);
		anim->drop();

		anim = new CSceneNodeScalePulseStaggeredAnimator(device->getTimer()->getTime(), vector3df(wep->scale, wep->length, wep->scale), vector3df(.1f),
			250U, (u32)(wep->lifetime*1000.f) - 500U, smgr);
		projIrr.node->addAnimator(anim);
		anim->drop();

		proj.set<BulletRigidBodyComponent>(shotRBC);
		proj.set<IrrlichtComponent>(projIrr);
		initializeHealth(proj, wep->damage);
		auto hp = proj.get_mut<HealthComponent>();
		hp->healthResistances[IMPACT] = 1.f;
		if (ship.has<FactionComponent>()) proj.set<FactionComponent>(*ship.get<FactionComponent>());
		proj.add(gameController->firedBy(), wepId);
		proj.add(gameController->doNotCollideWith(), ship);
		auto shieldComp = wepId.get_mut<PhysShieldComponent>();
		shieldComp->shield = proj;
		gameController->registerDeathCallback(proj, _shieldImplodeCallback);
		return true;
	}
	return false;
}
